...
class SvgUri extends Component {
  ...
  constructor(props) {
    ...
    let svgXmlData = props.svgXmlData;
    let isloaded = false;
    if (typeof (props.source) === "string") {
      svgXmlData = svgIcon[props.source];
      isloaded = true;
    }
    this.state = { fill: props.fill, svgXmlData: svgXmlData };
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    let svgXmlData = nextProps.svgXmlData;
    if (nextProps.source) {
      let isloaded = false;
      if (typeof (nextProps.source) === "string") {
        svgXmlData = svgIcon[nextProps.source];
      }
      else {
  ...
  obtainComponentAtts({ attributes }, enabledAttributes) {
    ...
    if (!styleAtts.fill && this.state.fill) {
      styleAtts.fill = this.state.fill;
    }
    if (!styleAtts.opacity && this.props.opacity) {
      styleAtts.opacity = this.props.opacity;
    }
