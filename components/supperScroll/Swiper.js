...
export default class Swiper extends Component {
  ...
  static defaultProps = {
    ...
    loop: true,
    loadMinimal: true, // FIXED: If there`s to much images, the device maybe crashed. So make sure set this property as 'ture'!
    loadMinimalSize: 1,
    autoplay: false,
    autoplayTimeout: 2.5,
  ...
  renderPagination = () => {
    ...
    const indexStyle = Object.assign({
      backgroundColor: 'rgba(0,0,0,.5)', height: getResponsiveValue(26), alignItems: "center", justifyContent: "center", borderRadius: getResponsiveValue(13), paddingHorizontal: getResponsiveValue(14)
    }, this.props.indexStyle)
    const indexDot = <View key={this.state.index + 'index'} style={indexStyle}>
      <Text style={[{ color: '#ffffff', fontSize: getResponsiveValue(24) }]}>{(this.state.index + 1) + "/" + this.state.total}</Text>
    </View>
    dots.push(indexDot)
    return (
